...
*   **Linear**: bypass the entire darkroom pipeline and dump the scanner's or camera's decoded buffer as an untagged linear 16-bit TIFF. No normalization, exposure, colour management, flatfield, or sensor correction — just the raw data with lossless geometry (rotation/flip) applied. Supported sources:
    ...
    *   **Expansion**: scales the linear data before writing. The combo box shows source-appropriate options: Pakon F135/F235 default to 4×, Noritsu defaults to 16×, F335 and LinearRaw DNG default to off. Camera RAW, Coolscan NEF, and Flextight FFF files have no expansion option. Leave at the default unless you know why you need to change it. A clipping guard checks the selected factor against the source's actual data and automatically reduces it if the source's real bit depth doesn't support it — protecting highlight detail instead of flattening it to white. When a cap happens, it's never silent: the export status message notes how many files were affected, and each affected file's TIFF description records the reduced factor.
