...
With **Normalize** off (the default for a transparency), bounds are a fixed window, so the Survival sliders' effect is fully visible. With **Normalize** on, per-channel bounds are re-measured after the correction, which absorbs most of a plain survival-ratio gain — δ is what still shows. Turning Normalize on can make Fade Restoration look like it stopped working; it hasn't, the meter is just compensating.
