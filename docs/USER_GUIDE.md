...
*   **Green Survival** / **Blue Survival** (0.5 to 2.0, default 1.0): how much *this slide's* green and blue layers have survived, relative to red — the two per-image unknowns a profile can't supply. 1.0 means that layer has faded the same as red; below 1.0, more; above, less. Only the two ratios matter, not an absolute scale, since a uniform survival change is undone by normalization regardless.
