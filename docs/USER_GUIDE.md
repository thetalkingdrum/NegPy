...
*   **Scan clip**: names each channel whose source-scan pixels at or above sensor white pass the point where the shadow-neutral tie's own sample can start landing in the clipped pile instead of the true base (currently above 2%). Past that point the black point in that channel is estimated, not measured, so the colour balance is unreliable — and no edit can undo it, since the separation was destroyed in the scan.
