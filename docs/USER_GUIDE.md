...
*   **Profile**: the dye set's side-absorption ratios (δ), a property of the stock, not of any one frame. *None* means no side absorption. The bundled profiles (*Generic E6* and four named stocks) are computed from published spectral dye-density curves at 450/550/650 nm — Gschwind's narrowband bands — so they describe a Narrowband Scanner or Trichrome capture; a broadband scan's side absorption is different and much larger, and these numbers do not describe it. Custom `.toml` profiles live in `<Documents>/NegPy/fade/` (see [fade/README.md](../fade/README.md)). The slider button opens an editor for the six off-diagonal terms and the R/G/B measurement wavelengths (**Bands**) they were read at; the diagonal is fixed, since a profile is δ only. A profile with no bands doesn't appear at all — δ is meaningless without knowing the scanner it was measured on. If a Crosstalk matrix is also active for the same film process, δ is dropped from Fade Restoration — the unmix has already moved the data toward dye concentrations by then, where fading is a plain per-channel scale with no side absorption of its own to correct for. A hint below Strength says so; Green/Blue Survival are unaffected.
...
*   **Estimate** (magic-wand icon): reads this frame's own per-channel density span, unmixed by the selected Profile's δ first so the read is in dye concentration rather than measured density (skipping this step biases every ratio toward 1.0 — under-reporting fade, worse the more the slide has actually faded), and populates the two sliders above. A suggestion, not a lock — re-running it overwrites rather than accumulates, changing Profile clears it (it was read against that profile's δ), and a hint below reports the result or why it declined (channel spans too flat, in agreement, or the frame isn't a transparency). A legitimately monochromatic slide reads as faded here; override by hand if so.
*   **Strength** (0.0 to 1.0, default 1.0): how much of the restoration to apply. Defaults to full rather than off, unlike Crosstalk's own Strength — the real off-state is Green/Blue Survival sitting at 1.0, so this stays inert until a ratio is touched, and Estimate has a visible effect immediately instead of needing Strength raised separately. It changes what the analysis reads, so **re-run Batch Analysis** after changing it. A hint below reports when the restoration matrix can't be inverted safely at the current Strength/profile and falls back to Crosstalk alone.
...
*   **Mixing scans in one batch**: allowed, but the roll is what makes batch worth running. A frame that reads its own film edge keeps its own crop, and the roll supplies only what that frame could not measure. A frame that finds no edge at all is placed from the roll instead, so it takes the roll's width and tilt — the rescue on a consistent roll, and the risk in a selection of unrelated scans. Frames from one camera, holder and format pool best; mixed formats are safe while each frame reads its own edge.
...
*   **Export profile**: what the exported file is converted to and tagged with — a color space (`Same as Source`, `sRGB`, `Adobe RGB`, `ProPhoto RGB`, `P3 D65`, `Rec 2020`, or `Greyscale` for true B&W output), or an imported ICC profile for a printer or paper. `Same as Source` names the space it resolves to for the open frame; for an untagged scan that is the Adobe RGB the pipeline encodes to. An imported profile is not a preview-only proof: it is converted to and embedded in the file. Not available for JPEG XL output; see the Format note above.
*   **Import ICC** (the folder button on the COLOR MANAGEMENT header): copy a `.icc`/`.icm` into `~/NegPy/icc/`, where it joins both lists at once — no restart. A profile named after a built-in space (`sRGB.icc`) replaces that space's profile everywhere rather than appearing as a separate choice, and asks first.
*   **Proof on screen** (on by default): simulate the Export profile and Input ICC in the preview, so what you see matches what you get. Preview only — the export is unaffected either way. Turn it off to preview at full gamut, and a warning appears while the preview cannot show the export's color clipping.
