...
*   **Fade restoration** (`fade_strength` / `fade_alpha` / `fade_delta`, Transparency only): composes a second 3×3, `inv(F)`, into the same slot rather than adding a stage — `M_total = F_restore @ M_unmix` (`effective_crosstalk_matrix`). $F$ is built from each layer's surviving dye fraction $\alpha$ and the dye set's side-absorption ratios $\delta$ (`resolve_fade_matrix`), and, unlike the crosstalk unmix, is deliberately **not** row-normalized: fading changes each layer's neutral density, and that difference is the signal a row-normalized operator would erase. Strength scales the *parameters* toward the identity ($\alpha_\text{eff} = 1 + s(\alpha-1)$, $\delta_\text{eff} = s\delta$) before $F$ is built, not the output, so a partial application stays a physically coherent, less-faded state rather than a blend of two renders. $F$ is inverted directly; when it is singular or `cond(F)` exceeds 50 the stage falls back to the uncomposed matrix rather than amplifying noise.
