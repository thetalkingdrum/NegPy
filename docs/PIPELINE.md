...
*   **Fade restoration** (`fade_strength` / `fade_ratio_g` / `fade_ratio_b` / `fade_delta`, Transparency only): composes a second 3×3, `inv(F)`, into the same slot rather than adding a stage — `M_total = F_restore @ M_unmix` (`effective_crosstalk_matrix`). $F$ pins the red layer's surviving dye fraction at 1.0 and takes only the green/red and blue/red survival ratios, plus the dye set's side-absorption ratios $\delta$ (`resolve_fade_matrix`); a uniform scale of all three survival fractions is exactly absorbed by the per-channel stretch above, so it is not exposed as a free parameter. $\delta$ is treated as zero when no profile supplies one, so the ratios alone still give a real, diagonal-only correction. Unlike the crosstalk unmix, $F$ is deliberately **not** row-normalized: fading changes each layer's neutral density, and normalizing that away would erase the signal. Strength scales the ratios toward 1.0 and $\delta$ toward zero before $F$ is built, not the output, so a partial application stays a physically coherent, less-faded state. $F$ inverts directly; singular or `cond(F) > 50` falls back to the uncomposed matrix, and `fade_reject_reason` reports which so the failure is never silent. A crosstalk profile active for the same process describes the same physical side absorptions as $\delta$ — composing both would apply it twice — so `fade_delta_conflict_reason` drops $\delta$ from the fade factor whenever a same-mode crosstalk matrix is also active, leaving the survival ratios (which have no crosstalk equivalent) untouched. The two survival ratios are estimated on request (`features/process/fade.py`), never during render: a per-channel P1-P99 span read from the raw capture before any unmix, ratio taken directly from the span ratio, reported rather than applied when the spans are too flat, already agree, or the frame isn't a transparency.
