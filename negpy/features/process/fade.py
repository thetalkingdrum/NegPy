...
from negpy.features.exposure.normalization import percentile_from_sorted, prefilter_log_grid, sorted_channel_grid
...
def measure_channel_spans(image: ImageBuffer, roi: Optional[tuple], analysis_buffer: float) -> tuple[float, float, float]:
    ...
    unmix or fade composition, so the estimate cannot be derived from data its own
    correction has already touched. Independent of `LogNegativeBounds`: those are
    measured post-unmix (the feedback loop this avoids) and, on the default E-6
    transfer path, not measured at all (`transfer_bounds` is a fixed, content-blind
    window with no per-channel span to read)."""
...
def estimate_fade_ratios(
    image: ImageBuffer, process_mode: Optional[str], roi: Optional[tuple], analysis_buffer: float
    ...
    spans = measure_channel_spans(image, roi, analysis_buffer)
