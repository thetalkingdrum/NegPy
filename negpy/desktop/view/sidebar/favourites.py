...
class FavouritesSidebar(BaseSidebar):
    ...
    def _init_ui(self) -> None:
        row = QHBoxLayout()
        self.edit_btn = self._labeled_action("fa5s.sliders-h", " Edit Favorites", "Choose which controls appear here, and in what order")
        self.edit_btn.clicked.connect(self._open_editor)
        row.addWidget(self.edit_btn)
        row.addStretch()
        self.layout.addLayout(row)

        self.empty_hint = hint_label("No favorites yet — use Edit Favorites to pick the controls you reach for most.")
